...
from configuration.basic_config import ELASTICSEARCH_URL
from utils.utils import logging


def get_json_from_elk(file_name: str):
    while True:
        try:
        ...
        index_name = file_name.replace('*', datetime.now().strftime('%Y.%m.%d'))
        try:
...
def process(file_name: str):
    res = get_json_from_elk(file_name)
    if res and res.get('hits', {}).get('hits'):
        ...
        logging.error("❌ Not found on Elasticsearch")
